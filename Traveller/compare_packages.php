...
.compare-table th { padding:16px; font-family:'Fraunces',serif; font-size:16px; text-align:left; background:var(--teal-pale); color:var(--teal); border:1px solid var(--border); }
.compare-table td { padding:14px 16px; border:1px solid var(--border); vertical-align:top; font-size:14px; }
.compare-table tr:nth-child(even) td { background:#fafafa; }
.compare-table .row-label { font-weight:600; color:var(--ink-2); background:var(--sand); white-space:nowrap; }
.best { background: #f0fdf4 !important; }
</style>
</head>
<body>
...
<div class="container page-wrap">
  ...
  <form method="GET" id="compareForm" style="margin-bottom:20px; display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap">
  ...
  <div class="alert alert-info">Add at least 2 packages to compare. Browse <a href="packages.php">packages</a> and click ⚖️ to add them.</div>
  <?php else: ?>
  ...
  <table class="compare-table">
    ...
    <tbody>
      <tr>
        ...
        <?php foreach ($packages as $pkg): ?><td><?= htmlspecialchars($pkg['Destination']) ?></td><?php endforeach; ?>
      </tr>
      <tr>
        <td class="row-label">Agency</td>
        <?php foreach ($packages as $pkg): ?><td><?= htmlspecialchars($pkg['AgencyName']) ?></td><?php endforeach; ?>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td class="<?= $pkg['AgencyRating']==$maxRat?'best':'' ?>">
          ...
          <span class="stars"><?= str_repeat('★',round($pkg['AgencyRating'])) . str_repeat('☆',5-round($pkg['AgencyRating'])) ?></span>
      ...
      <tr>
        ...
        <?php foreach ($packages as $pkg): ?>
        <td><?= $pkg['Discount'] ? '<span class="alert alert-success" style="padding:4px 8px; font-size:12px">🏷️ '.htmlspecialchars($pkg['Discount']).'</span>' : '<span class="text-muted">None</span>' ?></td>
        <?php endforeach; ?>
      </tr>
      <tr>
        <td class="row-label">Book</td>
        <?php foreach ($packages as $pkg): ?>
        <td><a href="/COS221-PA5/packages_detail.php?id=<?= $pkg['PackID'] ?>" class="btn btn-primary btn-sm">View & Book →</a></td>
        <?php endforeach; ?>
      </tr>
    </tbody>
  </table>
