...
<body>
    ...
    <div class="container page-wrap">
            ...
            <div>
                ...
                <?php if ($activeTrip): ?>
                    <div class="alert alert-success" style="font-size:15px; margin-bottom:28px">
                        ✈️ You're currently on a trip!
                        &nbsp;<strong><?= htmlspecialchars($activeTrip['PackName']) ?></strong>
                        → <?= htmlspecialchars($activeTrip['Destination']) ?> &nbsp;·&nbsp;
                        <?= $activeTrip['DaysLeft'] ?> day<?= $activeTrip['DaysLeft'] != 1 ? 's' : '' ?> left.
                        <a href="../packages_detail.php?id=<?= $activeTrip['PackID'] ?>" class="btn btn-sm btn-outline"
                            style="margin-left:12px">View Package</a>
                    </div>
                <?php endif; ?>
      ...
      ">
                        <div
                            style="position:absolute; right:40px; top:50%; transform:translateY(-50%); font-size:120px; opacity:.12; pointer-events:none">
                            <?= destIcon($nextTrip['Destination'], $destIcons) ?>
                        </div>

                        <div
                            style="font-size:12px; font-weight:600; text-transform:uppercase; letter-spacing:1px; opacity:.7; margin-bottom:8px">
                            Next Adventure
                        </div>
                        ...
                        <div style="display:flex; gap:32px; align-items:center; flex-wrap:wrap">
                            <div>
                                ...
                                <div style="opacity:.7; font-size:13px; margin-top:4px">
                                    <?= date('d M Y', strtotime($nextTrip['From'])) ?> –
                                    <?= date('d M Y', strtotime($nextTrip['To'])) ?>
                                </div>
                            </div>
                            <div style="display:flex; gap:12px; margin-left:auto">
                                <span class="pack-badge badge-<?= strtolower($nextTrip['Class']) ?>"
                                    style="border:1px solid rgba(255,255,255,.4); background:rgba(255,255,255,.15); color:#fff">
                                    <?= $nextTrip['Class'] ?>
                                </span>
                                <a href="../packages_detail.php?id=<?= $nextTrip['PackID'] ?>" class="btn btn-sm"
                        ...
                        <div style="display:flex; flex-direction:column; gap:12px; margin-bottom:40px">
                            <?php foreach (array_slice($upcomingTrips, 1) as $trip): ?>
                                <div class="card card-body" style="display:flex; align-items:center; gap:16px">
                                    ...
                                    <a href="../packages_detail.php?id=<?= $trip['PackID'] ?>"
                ...
                <!-- ── PAST ADVENTURES ── -->
                <?php if ($pastTrips): ?>
                    <h2 style="font-family:'Fraunces',serif; font-size:20px; margin-bottom:16px; margin-top:40px">Past
                        Adventures</h2>
                    <div class="card">
                        <table class="table">
                            <thead>
