...
$sql = "
    ...
    WHERE 1=1";
$params = [];
if ($search) { $sql .= " AND (to2.Name LIKE ? OR to2.City LIKE ? OR d.Name LIKE ?)"; $params = ["%$search%","%$search%","%$search%"]; }
...
<div class="container page-wrap">
  <div class="sidebar-layout">
    ...
    <div>
      <div class="page-header"><h1>🍽️ Restaurants</h1><p><?= count($restaurants) ?> restaurants</p></div>
      <form method="GET" class="filter-bar">
        <div class="form-group"><label>Search</label><input class="form-control" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Restaurant or city…"></div>
      ...
      <div class="package-grid">
        ...
        <div class="pack-card">
          ...
          <div class="pack-card-body">
            ...
            <?php if ($r['AvgRating']): ?>
            <div class="text-sm" style="color:var(--gold); margin-bottom:6px"><?= str_repeat('★',round($r['AvgRating'])) . str_repeat('☆',5-round($r['AvgRating'])) ?> <?= $r['AvgRating'] ?></div>
            <?php endif; ?>
            <div class="text-sm text-muted">🕐 <?= substr($r['TimeOpen'],0,5) ?> – <?= substr($r['TimeClose'],0,5) ?></div>
            <?php if ($r['MenuSample']): ?>
            <div class="text-sm text-muted" style="margin-top:6px">Menu: <?= htmlspecialchars(mb_strimwidth($r['MenuSample'],0,80,'…')) ?></div>
            <?php endif; ?>
          </div>
        </div>
        <?php endforeach; ?>
