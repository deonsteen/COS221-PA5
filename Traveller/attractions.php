...
$params = [];
if ($search) {
  $sql .= " AND (to2.Name LIKE ? OR to2.City LIKE ?)";
    ...
    <div class="sidebar-layout">
      ...
      <div>
        <div class="page-header">
          ...
          <p><?= count($attractions) ?> attractions</p>
        </div>
        <form method="GET" class="filter-bar">
          <div class="form-group"><label>Search</label><input class="form-control" name="search"
              value="<?= htmlspecialchars($search) ?>" placeholder="Attraction or city…"></div>
          ...
          <?php foreach ($attractions as $a): ?>
            <div class="pack-card">
              ...
              <div class="pack-card-body">
                ...
                <div class="pack-dest">📍 <?= htmlspecialchars($a['City']) ?>, <?= htmlspecialchars($a['DestName']) ?>
                </div>
                <?php if ($a['AvgRating']): ?>
                  <div class="text-sm" style="color:var(--gold); margin-bottom:6px">
                    <?= str_repeat('★', round($a['AvgRating'])) . str_repeat('☆', 5 - round($a['AvgRating'])) ?>
                    <?= $a['AvgRating'] ?></div>
                <?php endif; ?>
                <div class="text-sm text-muted">🕐 <?= substr($a['TimeOpen'], 0, 5) ?> – <?= substr($a['TimeClose'], 0, 5) ?>
                </div>
              </div>
              <div class="pack-footer">
                <div class="pack-price">
                  <?= $a['Price'] > 0 ? 'R' . number_format($a['Price'], 0) : '<span class="text-muted" style="font-size:14px">Free entry</span>' ?>
                </div>
              </div>
            </div>
          <?php endforeach; ?>
