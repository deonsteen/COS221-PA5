      ...
      <div>
        ...
        <form method="GET" class="filter-bar">
          <div class="form-group">
            <label>Search</label>
            <input class="form-control" type="text" name="search" placeholder="Package, destination, agency…"
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="dest">
              ...
              <?php foreach ($dests as $d): ?>
                <option value="<?= htmlspecialchars($d) ?>" <?= $dest === $d ? 'selected' : '' ?>><?= htmlspecialchars($d) ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <input class="form-control" type="number" name="max" placeholder="Any"
              value="<?= htmlspecialchars($_GET['max'] ?? '') ?>">
          </div>
          <div class="form-group">
            <label>Max Days</label>
            <input class="form-control" type="number" name="dur" placeholder="Any"
              value="<?= htmlspecialchars($_GET['dur'] ?? '') ?>">
          </div>
          <div class="form-group">
            <label>Sort by</label>
        ...
        <?php if (isset($_GET['compare_msg'])): ?>
          <div class="alert alert-info">Package added to compare. <a href="compare_packages.php">View comparison →</a>
          </div>
        <?php endif; ?>
              ...
              <div class="pack-card">
                ...
                <div class="pack-card-body">
                  ...
                  <?php if ($pkg['DiscDetails']): ?>
                    <div class="alert alert-success text-sm" style="padding:6px 10px; margin-bottom:8px">🏷️
                      <?= htmlspecialchars($pkg['DiscDetails']) ?></div>
                  <?php endif; ?>
                </div>
                <div class="pack-footer">
                  ...
                  <div style="display:flex; gap:6px">
                    ...
                    <a href="/COS221-PA5/packages_detail.php?id=<?= $pkg['PackID'] ?>" class="btn btn-primary btn-sm">View
                      →</a>
                  </div>
                </div>
              </div>
